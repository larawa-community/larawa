...
| 12 | PostgreSQL and Redis can be enabled through environment variables. | `.env.postgres.example` enables the `postgres` profile with database-backed services, and `.env.redis-postgres.example` adds Redis-backed cache, queues, and sessions. `scripts/verify.sh --with-postgres-up` starts an isolated stack and asserts `/healthz` reports `pgsql`. |
